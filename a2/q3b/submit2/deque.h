...
namespace cs246e {
  template<typename T> class deque {
      ...
      */
      ...
      deque(deque &other): n{0}, cap{other.cap}, min_idx{2}, max_idx{2} {
         static node<T> *tmp[300];
         tmp[2] = new node<T>();
         for (auto &val: other){ // <- Addes each value to other to the next spot in tmp
            if (tmp[max_idx]->right_full()){ // <- If the current node is used up, go to the next one
               max_idx = max_idx + 1;
