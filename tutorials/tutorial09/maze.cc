...
Maze::Maze(int rows, int cols, bool curses, bool walls): sizeX{cols}, sizeY{rows}{
  srand(time(NULL));
...
void Maze::recursiveWalls(int x0, int y0, int x1, int y1){
  ...
  recursiveWalls(x0, y0, dir==0?wallLoc-1:x1, dir==0?y1:wallLoc-1);
  recursiveWalls(dir==0?wallLoc+1:x0, dir==0?y0:wallLoc+1,x1,y1);
}
...
State Maze::getState(int row, int col) {
 try{
  return grid.at(row).at(col).state;
